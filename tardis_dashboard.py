...
internationalgare = ('BARCELONA', 'MADRID', 'STUTTGART', 'LAUSANNE', 'GENEVE', 'ITALIE', 'ZURICH', 'FRANCFORT')
...
with st.container(border=True):
    ...
    df_filtered = df
    df_filtre = df_filtered.groupby('month')['Nombre de trains en retard au départ'].agg(['mean', 'max', 'min', 'median', 'sum']).reset_index()
...
with st.container(border=True):
    ...
    df_filtered = df
    df_filtre = df_filtered.groupby('Gare de départ')['Retard moyen des trains en retard à l\'arrivée'].agg(['mean', 'max', 'min', 'median', 'sum']).reset_index()
    ...
    with left:
        ...
        ax2.set_title('Average delay for the top 20 train stations based on the number of trains delayed at arrival')
        ax2.set_xticks(range(len(X_array)))
        wrapped_labels = [(label[:bp] + '\n' + label[bp+1:] if (bp := label.rfind(' ', 0, 15)) != -1 else label[:15] + '\n' + label[15:]) if len(label) > 15 else label for label in X_array]
...
with st.container(border=True):
    ...
    with left:
        ...
        st.markdown('(the red line is the average delay at arrival of ll the train stations)')
        st.markdown('')
        gare1 = st.selectbox('Departure Station', df_gare, key='gare1_compare', width=250)
        if gare1 in list(df_gare):
                df_gare2 = list(df_gare)
                df_gare2.remove(gare1)
        gare2 = st.selectbox('Arrival station', df_gare2, index=1, key='gare2_compare', width=250)
    df_filtered = df
    df_filtre = df_filtered[df_filtered['Gare de départ'].isin([gare1, gare2])].groupby('Gare de départ')['Retard moyen des trains en retard à l\'arrivée'].mean().reset_index()
    df_filtre = df_filtre.sort_values(by='Retard moyen des trains en retard à l\'arrivée', ascending=False)
    X_array = df_filtre['Gare de départ']
    ...
    with right:
        ...
        ax2.set_xlabel('Departure station')
        ax2.set_ylabel('Average delay (min)')
        ax2.set_title(f'Average delay comparison')
        ax2.axhline(y=df['Retard moyen des trains en retard à l\'arrivée'].mean(), color='red', linestyle='-', linewidth=2, label='Average')
        ax2.set_xticks(range(len(X_array)))
...
with st.container(border=True):
    ...
    with col_form1:
            ...
            if garedeinput in list(df_gare):
                df_gare2 = list(df[df["Gare de départ"]== garedeinput]["Gare d'arrivée"].drop_duplicates())
                
            
            garearrinput = st.selectbox('Arrival Station', df_gare2, width=300)
            dureedetruc = st.number_input('Journey Duration (minutes)', width=300, value=0, min_value=0)
            ...
            if submit_button:
                ...
                prediction = round(predict_delay(model, garedeinput, garearrinput, dureedetruc, dateinput.month, dateinput.year, natio, internatio), 2)
    ...
    st.markdown('In this context the score MAE is the average error in minutes of the model compared to the reality') 
    st.markdown('The score RMSE is derived from the MAE score but the mistakes have more influence on the score based on their size')
    st.markdown('the R² score is a statistical measure that represents the proportion of the variance in a dependent variable that is explained by one or more independent variables in a regression model.')